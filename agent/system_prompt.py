...
@register_tool("mobile_use_without_open")
class MobileUseWithoutOpen(BaseTool):
    parameters = {
        "properties": {
            "action": {
                "description": """
...
""".strip(),
            ...
            "text": {
                "description": "Required only by `action=key` and `action=type`.",
                "type": "string",
            },
            "time": {
...
def get_agent_system_prompt_for_qwen_vl_en(image_width, image_height) -> list[dict]:
    mobile_use = MobileUseWithoutOpen(
    ...
    system_message["content"].append({"text": output_format})
    return ". ".join([item["text"] for item in system_message["content"]])


if __name__ == "__main__":
    print(get_agent_system_prompt_for_qwen_vl_en(1, 1))
